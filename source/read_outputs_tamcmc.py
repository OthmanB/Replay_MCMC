...
def getmodel_bin(model_name, params, plength, xr, cpp_path='cpp_prg', outdir='tmp/', read_output_params=True):
	'''
		Build a model using the model_name and the params parameters.
		Warning: Requires getmodel v1.84.6
		...
		xr: A vector of 3 values used to generate a x-axis by getmodel [xmin, xmax, resolution]
...
def bin2txt(dir_tamcmc_outputs, process_name, phase='A', chain=0, first_index=0, period=1, erase_tmp=True, cpp_path='cpp_prg/', outdir='tmp/', get_plength=False):
	'''
		...
		update on 1 Dev 2022: adding read_plength: If True, we return also plength
	'''
...
def version():
	print('read_output_tacmcmc version 2')
